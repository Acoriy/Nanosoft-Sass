...
import { Skeleton } from "@/components/ui/skeleton";
import { useIsMobile } from "@/hooks/use-mobile";
import PriceForm, { PriceFormValues } from "@/components/PriceForm";
...
const PriceAdmin = () => {
  ...
  const handleFormSubmit = async (values: PriceFormValues) => {
    console.log("Données envoyées depuis PriceForm :", values);
    try {
      if (editingPrice) {
        if (!editingPrice.id) {
          toast.error("خطأ في تحديث السعر: معرف السعر غير موجود");
        ...
        toast.success("تم تحديث السعر بنجاح");
          ...
          const newId = await addPrice(values);
          setPrices([...prices, { id: newId, ...values }]);
        toast.success("تم إضافة السعر بنجاح");
      }
      setIsOpen(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="accounting" onValueChange={setSelectedCategory} value={selectedCategory}>
        <TabsList className="mb-4 flex flex-wrap h-fit">
          {serviceCategories.map((category) => (
            <TabsTrigger key={category.id} value={category.id} className="flex items-center gap-2">
              <span>{category.icon}</span>
                          ...
                          filteredPrices.map((price) => (
                            <TableRow key={price.id}>
                              ...
                              <TableCell className="flex gap-2 justify-end">
                                ...
                                <Button variant="destructive" size="icon" onClick={() => handleDelete(price.id!)}>
                                  <Trash2 size={16} />
                                </Button>
                              </TableCell>
                            </TableRow>
