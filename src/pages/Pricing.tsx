...
const Pricing = () => {
  ...
  const [pricePlans, setPricePlans] = useState<Price[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  // Nouvel état pour la devise d'affichage
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time updates - without toast notifications here
    const unsubscribe = subscribeToPricesUpdates((updatedPrices) => {
      ...
      if (filteredPrices.length > 0) {
        console.log(
          `Received ${filteredPrices.length} prices for ${selectedService} in real-time`
        );
        setPricePlans(filteredPrices);
        setIsLoading(false);
      }
  ...
  return (
    <div className="pt-24 pb-20 min-h-screen bg-gradient-to-b from-white to-gray-50">
      <div className="container mx-auto px-4">
        ...
        {/* Section de sélection de service et de devise */}
              ...
              {pricePlans.length > 0 ? (
                pricePlans.map((plan) => (
                  <motion.div
                    ...
                    transition={{ type: "spring", stiffness: 400, damping: 17 }}
                  >
                    {/* Transmission de displayCurrency à PriceCard */}
                    <PriceCard
                      price={plan}
        ...
        {/* Reste de la page (Contact, FAQ, etc.) */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.5 }}
