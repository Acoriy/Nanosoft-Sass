...
// Schéma de validation incluant le champ priceUSD
export const priceSchema = z.object({
  title: z.string().min(3, { message: "العنوان يجب أن يحتوي على 3 أحرف على الأقل" }),
  price: z.string().min(1, { message: "السعر مطلوب" }),
  priceUSD: z.string().min(1, { message: "السعر بالدولار مطلوب" }),
  currency: z.string().min(1, { message: "العملة مطلوبة" }),
  period: z.string().min(1, { message: "الفترة مطلوبة" }),
  features: z.string().min(5, { message: "المميزات يجب أن تحتوي على 5 أحرف على الأقل" }),
  isPopular: z.boolean().default(false),
...
interface PriceFormProps {
  price?: any; // Vous pouvez affiner le typage en fonction de votre besoin
  onSubmit: (data: PriceFormValues) => void;
  onCancel: () => void;
  isSubmitting?: boolean;
}
...
const PriceForm: React.FC<PriceFormProps> = ({ price, onSubmit, onCancel, isSubmitting = false }) => {
  ...
  const handleSubmit = form.handleSubmit((data) => {
    // On appelle la fonction onSubmit avec les données du formulaire
    try {
      onSubmit(data);
    } catch (error) {
      console.error("خطأ في إرسال النموذج:", error);
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                {...form.register("priceUSD")}
                className="text-right"
                aria-invalid={!!form.formState.errors.priceUSD}
              />
              {form.formState.errors.priceUSD && (
                <p className="text-red-500 text-sm" role="alert">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={handleFeaturesChange}
              className="min-h-[100px] text-right"
              aria-invalid={!!form.formState.errors.features}
            />
            {form.formState.errors.features && (
              <p className="text-red-500 text-sm" role="alert">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Checkbox
              ...
              aria-checked={form.getValues("isPopular")}
            />
          </div>
        </CardContent>

        <CardFooter className="flex justify-between">
          <Button type="submit" disabled={isSubmitting} aria-label={isSubmitting ? "جارٍ الحفظ" : price ? "تحديث الباقة" : "إنشاء الباقة"}>
            {isSubmitting ? "جاري الحفظ..." : price ? "تحديث" : "إنشاء"}
          </Button>
          <Button type="button" variant="outline" onClick={onCancel} aria-label="إلغاء العملية">
            إلغاء
          </Button>
        </CardFooter>
      </form>
