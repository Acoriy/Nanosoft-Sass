...
const HomeBlogSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        setLoading(true);
        const fetchedBlogs = await getBlogs();
        // Filtrer uniquement les blogs publiés et limiter à 3
        const publishedBlogs = fetchedBlogs
          .filter((blog) => blog.published)
          .sort(
            (a, b) =>
  ...
  return (
    <section className="py-16 bg-white" aria-labelledby="home-blog-title">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <Badge
            variant="outline"
            className="px-3 py-1 text-nanosoft-primary border-nanosoft-primary mb-3"
          >
            المدونة
          </Badge>
          ...
          >
            {blogs.map((blog, index) => (
              <motion.div
                ...
                transition={{ duration: 0.5, delay: index * 0.1 }}
              >
                <Link to={`/blog/${blog.id}`} aria-label={`قراءة المقال ${blog.title}`}>
                  <Card className="overflow-hidden h-full hover:shadow-lg transition-shadow duration-300 flex flex-col">
                    <div className="relative overflow-hidden h-48">
                      <img
                        src={
